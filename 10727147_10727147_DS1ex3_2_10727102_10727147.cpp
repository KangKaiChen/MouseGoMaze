...
int pause(   numpty &num, bool & run  ){
 int total = 0,num1 = -1 , num2 = -1;
 char op = '+', op2 = '\0';// 前加減後程除 
 bool have = false;
 while(  num != NULL && run  )  { //括號 
    ...
    if( num->num != -1   ) {
    ...
    else  if( getslevel( num->column ) == 1  ) {
     	if(  op2 == '*'  ) 
     		num1 = num1*num2;
     	else if(  op2 == '/'  ) {
     		if( num2 == 0  ){
	 ...
	 else if(   getslevel( num->column ) == 2    ){
     	if(  op2 == '*'  ) 
     		num1 = num1*num2;
     	else if(  op2 == '/'  ) {
     		num1 = num1/num2;  
...
int main(int argc, char** argv) {
	...
	do {
	  ...
	  if( ( command == '1' || command == '2' || command == '3' )  && ch == '\n'  ) {
	  	cout << "Input:" ;
	    Read( num, run);
        if( gnum != gnum2   ) {
    	  run = false;
